...
function parseURL(req_url) {
  ...
  if (!match[1]) req_url = (match[4] === '443' ? 'https:' : 'http:') + '//' + req_url;
...
function onProxyResponse(proxy, proxyReq, proxyRes, req, res) {
  ...
  if (!requestState.redirectCount_) res.setHeader('x-request-url', requestState.location.href);

  // Handle redirects
  if ([301, 302, 303, 307, 308].includes(statusCode)) {
    const locationHeader = proxyRes.headers.location;
    if (locationHeader) {
      const resolved = url.resolve(requestState.location.href, locationHeader);
      const parsed = parseURL(resolved);
      if (parsed && statusCode !== 307 && statusCode !== 308) {
        requestState.redirectCount_ = (requestState.redirectCount_ || 0) + 1;
        if (requestState.redirectCount_ <= requestState.maxRedirects) {
          req.method = 'GET';
          delete req.headers['content-length'];
          delete req.headers['content-type'];
          requestState.location = parsed;
...
function proxyRequest(req, res, proxy) {
  ...
  const proxyOptions = {
    ...
    buffer: {
      pipe(proxyReq) {
        const origOn = proxyReq.on;
        proxyReq.on = function(eventName, listener) {
          if (eventName !== 'response') return origOn.call(this, eventName, listener);
          return origOn.call(this, 'response', function(proxyRes) {
            if (onProxyResponse(proxy, proxyReq, proxyRes, req, res)) {
              try { listener(proxyRes); } catch (err) { proxyReq.emit('error', err); }
            }
          });
        };
  ...
  try { proxy.web(req, res, proxyOptions); } catch (err) { proxy.emit('error', err, req, res); }
}
  ...
  return function(req, res) {
    ...
    if (req.url.startsWith("/?url=")) {
      try {
        ...
        console.error("Failed to parse ?url param:", err);
    ...
    req.corsAnywhereRequestState = {
      getProxyForUrl: options.getProxyForUrl || getProxyForUrl,
      maxRedirects: options.maxRedirects || 5,
      corsMaxAge: options.corsMaxAge || 0,
    };
    ...
    if (!location) {
      ...
      res.end('Invalid host or URL');
      return;
    }

    req.corsAnywhereRequestState.location = location;
    req.corsAnywhereRequestState.proxyBaseUrl = (req.connection.encrypted ? 'https://' : 'http://') + req.headers.host;
...
exports.createServer = function createServer(options) {
  options = options || {};
  const proxy = httpProxy.createServer(options.httpProxyOptions || { xfwd: true, secure: process.env.NODE_TLS_REJECT_UNAUTHORIZED !== '0' });
  const requestHandler = getHandler(options, proxy);

  const server = options.httpsOptions
    ? require('https').createServer(options.httpsOptions, requestHandler)
